...
COPY /lib/install_yottadb.sh /opt/qewd
COPY /lib/gde.txt /opt/qewd
RUN chmod +x /opt/qewd/install_yottadb.sh

RUN cd /opt/qewd

# Install app dependencies
COPY /lib/package.json /opt/qewd
RUN npm install
...
# Bundle app source
COPY /lib/ydb /opt/qewd
COPY /lib/qewd.js /opt/qewd

RUN chmod +x /opt/qewd/ydb

RUN mkdir /opt/qewd/mapped

COPY /helm/. /opt/qewd/mapped 
